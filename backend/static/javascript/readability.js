...
/*(function(){
	...
	document.body.className = "style-apertura";
	objOverlay.className = "style-apertura";
...
*/
function grabArticle_new(content) {
	try{
	alert('transform start');
	var allParagraphs = $(content.val()).find("p");
	alert('val:'+ $($(content).val()));
	var topDivCount = 0;
	var topDiv = null;
	var topDivParas;
	...
	alert($(content.val()).find("body"));
	var body = $(content.val()).find("body");
	
	// Replace all doubled-up <BR> tags with <P> tags, and remove fonts.
	var pattern =  new RegExp ("<br/?>[ \r\n\s]*<br/?>", "g");
	document.body.innerHTML = $(content.val()).find("body").html();//..innerHTML.replace(pattern, "</p><p>").replace(/<\/?font[^>]*>/g, '');
	return $(content.val()).find("body").html();
	} catch(e){
		alert('e1:'+e);
	}
}
	
function grabArticle(content) {
		alert('transform start');
	var allParagraphs = $(content.val()).find("p");
	var topDivCount = 0;
	var topDiv = null;
	...
	articleTitle.innerHTML = content.title;
	...
	alert("score:"+ parentNode.readability.contentScore);

	// Assignment from index for performance. See http://www.peachpit.com/articles/article.aspx?p=31567&seqNum=5 
	for(nodeIndex = 0; (node = content.getElementsByTagName('*')[nodeIndex]); nodeIndex++)
	...
	for (var k=0;k < content.styleSheets.length; k++) {
		if (content.styleSheets[k].href != null && content.styleSheets[k].href.lastIndexOf("readability") == -1) {
			content.styleSheets[k].disabled = true;
		}
	}

	// Remove all style tags in head (not doing this on IE) :
	var styleTags = content.getElementsByTagName("style");
...
function clean(e, tags, minWords) {
	var targetList = e.etElementsByTagName( tags );
